...
export class AuthController {
  ...
  @Public()
  @Post('api/login')
  async apiLogin(@Body() body: { username: string; password: string }) {
    try {
      ...
      if (!user) {
        this.logger.warn(
          `Failed API login attempt for username: ${body.username}`,
        );
      ...
      this.logger.error(`API login error: ${error.message}`, error.stack);
      throw error;
    }
  }
